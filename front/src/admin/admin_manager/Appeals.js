...
function Appeals() {
  ...
  const validateInputs = () => {
    ...
    const documentStatusTouched = documentStatus !== null && documentStatus?.value !== 0;

    const extraFieldsTouched =
      passportPinTouched || passportNumberTouched || descriptionTouched || documentStatusTouched;

    // Extra maydonlarga to‘liq kiritilganmi (agar foydalanuvchi boshlagan bo‘lsa)
    const extraDocumentValid = !extraFieldsTouched || (
          ...
          <tbody>
            {appeals.map((appeal, index) => (
              ...
              >
                ...
                <td
                  className={`border border-gray-200 px-1 py-1 text-[14px] ${appeal.documentStatus === 1 && "bg-yellow-500"
